...
function Accountstudent() {
  ...
  const berror = (
    <Message showIcon type="error" header="Error" closable>
      Your email Id is doesn't exist, Go and Take admission
    </Message>
  );
  const cerror = (
    <Message showIcon type="error" header="Error" closable>
  ...
  const sendOtp = async () => {
    try {
      setLoada(true);
      const res = await fetch("/sendotpst", {
        method: "POST",
        headers: {
  ...
  const checkOtp = async () => {
    try {
      setLoadb(true);
      const res = await fetch("/checkotpst", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const createid = async () => {
    try {
      ...
      if (password == cpassword) {
        const res = await fetch("/studentcreateaccount", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
    ...
    <>
      <div className={styles.container}>
        ...
        <div>
          ...
          {loadc === false
            ? userbtn && (
                <button onClick={createid} className={styles.login}>
                  Create account
                </button>
              )
            : loader}
          {otpbtn ||
            (verifyOtp && (
              <h4 onClick={sendOtp} className={styles.forgot}>
                Resend OTP
              </h4>
            ))}
        </div>
